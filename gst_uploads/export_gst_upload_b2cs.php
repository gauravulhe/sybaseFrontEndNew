...
$sql = "SELECT tbl.*, cod.cod_desc
        FROM $com_dbf.sales.$tblName AS tbl
        INNER JOIN catalog..codecat AS cod
        ON
        cod.cod_code = tbl.ups_uom AND
        cod.cod_prefix = 6
        WHERE (ups_state_cd != '27' AND ups_net_amt <= 250000) OR (ups_state_cd = '27' AND ups_net_amt > 0) AND ups_gstin_no != 'NULL' AND ups_sfx IN ('$inv_type') ORDER BY ups_bil_dt ASC";
