...
$sql = "SELECT ups_state_cd,ups_state_nm,ups_gst_rate,ups_taxable_amt,ups_cess_amt FROM $com_dbf.sales.$tblName WHERE (ups_state_cd != '27' AND ups_net_amt <= 250000) OR (ups_state_cd = '27' AND ups_net_amt > 0)";
...
echo '<th colspan="4">Summary For B2CS(7)</th>';
echo '</tr>';
echo '</table>';
echo '<table border="1">';
echo '<tr class="bg-color">';
...
$total_cess = 0;
    ...
    foreach ($rows as $row) {
        echo '<tr>';
            ...
            echo '<td>'.$row['ups_taxable_amt'].'</td>';
            echo '<td>'.$row['ups_cess_amt'].'</td>';
        echo '</tr>';
