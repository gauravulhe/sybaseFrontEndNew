<?php 
    ...
    if (!empty($_POST['submit'])) {
        ...
        if ($passActn == 'u' && $frm_nm == 'worder') { 
            $query = "UPDATE $UserComDbf.invac.pohdr SET poh_po_dt = '$poh_po_dt', poh_supcd = '$poh_supcd', poh_sup_accd = '$poh_sup_accd', poh_stax_cd = $poh_stax_cd, poh_stax_per = $poh_stax_per, poh_excise_cd = $poh_excise_cd, poh_dlv_dest = '$poh_dlv_dest', poh_disc = $poh_disc, poh_paycr_days = $poh_paycr_days, poh_crdisc = $poh_crdisc, poh_bank = '$poh_bank', poh_pmnt_terms = '$poh_pmnt_terms', poh_po_type = $poh_po_type, poh_addl_rmk = '$poh_addl_rmk', poh_exp_dt = '$poh_exp_dt', poh_gst_cd = $poh_gst_cd, poh_gst_per = $poh_gst_per, poh_igst_per = $poh_igst_per, poh_sgst_per = $poh_sgst_per, poh_cgst_per = $poh_cgst_per, poh_ugst_per = $poh_ugst_per WHERE poh_com = $poh_com AND poh_unit = $poh_unit AND poh_fyr = $poh_fyr AND poh_po_no = $poh_po_no";

            $sql_result = odbc_exec($conn, $query);
                if ($sql_result) { 

                    // UPDATE into podet
                    $queryPod = "UPDATE $UserComDbf.invac.podet SET pod_com = $pod_com, pod_unit = $pod_unit, pod_fyr = $pod_fyr, pod_po_no = $pod_po_no, pod_po_srl = $pod_po_srl, pod_item = '$pod_item', pod_tech_spec = '$pod_tech_spec', pod_rate = $pod_rate, pod_ord_qty = $pod_ord_qty, pod_tolerance = $pod_tolerance, pod_chpt_id = $pod_chpt_id WHERE pod_com = $poh_com AND pod_unit = $poh_unit AND pod_fyr = $poh_fyr AND pod_po_no = $poh_po_no AND pod_po_srl = $pod_po_srl";
            ...
            }else if($passActn == 'e' && $frm_nm == 'worder') {  
            ...
            if (!empty($row)) {
                ...
                $query = "INSERT INTO $UserComDbf.invac.pohdr (poh_com, poh_unit, poh_fyr, poh_po_no, poh_po_dt, poh_supcd, poh_sup_accd, poh_stax_cd, poh_stax_per, poh_excise_cd, poh_dlv_dest, poh_disc, poh_paycr_days, poh_crdisc, poh_bank, poh_pmnt_terms, poh_po_type, poh_addl_rmk, poh_exp_dt, poh_gst_cd, poh_gst_per, poh_igst_per, poh_sgst_per, poh_cgst_per, poh_ugst_per) VALUES ($poh_com, $poh_unit, $poh_fyr, $poh_po_no, '$poh_po_dt', '$poh_supcd', '$poh_sup_accd', $poh_stax_cd, $poh_stax_per, $poh_excise_cd, '$poh_dlv_dest', $poh_disc, $poh_paycr_days, $poh_crdisc, '$poh_bank', '$poh_pmnt_terms', $poh_po_type, '$poh_addl_rmk', '$poh_exp_dt', $poh_gst_cd, $poh_gst_per, $poh_igst_per, $poh_sgst_per, $poh_cgst_per, $poh_ugst_per)";

                $sql_result = odbc_exec($conn, $query);
                
                if ($sql_result) {

                    $updateQuery = "UPDATE $UserComDbf.invac.parinv SET par_lval = $poh_po_no WHERE par_com = $poh_com AND par_unit = $poh_unit AND par_tbl = 'pohdr' AND par_col = 2 AND par_fyr = $poh_fyr";

                    $sql_result_update_parinv = odbc_exec($conn, $updateQuery);

                    // insert into podet
                    $queryPod = "INSERT INTO $UserComDbf.invac.podet (pod_com, pod_unit, pod_fyr, pod_po_no, pod_po_srl, pod_item, pod_tech_spec, pod_rate, pod_ord_qty, pod_tolerance, pod_chpt_id) VALUES ($pod_com, $pod_unit, $pod_fyr, $pod_po_no, $pod_po_srl, '$pod_item', '$pod_tech_spec', $pod_rate, $pod_ord_qty, $pod_tolerance, $pod_chpt_id)";
                    ...
                    if ($sql_resultPod) {     
                        ...
                        for ($i=0; $i < count($pdc_id); $i++) { 
                            ...
                            $queryPdcomm = "INSERT INTO $UserComDbf.invac.pdcomm (pdc_com, pdc_unit, pdc_fyr, pdc_po_no, pdc_po_srl, pdc_id, pdc_tag, pdc_amt, pdc_sys_dt) VALUES ($pdc_com, $pdc_unit, $pdc_fyr, $pdc_po_no, $pdc_po_srl, $pdc_id[$i], $pdc_tag[$i], $pdc_amt[$i], '$pdc_sys_dt')";
                        ...
                        if ($sql_resultPdcomm) {
                            ...
                            for ($j=0; $j < count($pdd_sch_dt); $j++) { 
                                $pdd_sch_dt[$j] = date('Y-m-d 00:00:00', strtotime($pdd_sch_dt[$j]));
                                $queryPddet = "INSERT INTO $UserComDbf.invac.pddet (pdd_com, pdd_unit, pdd_fyr, pdd_po_no, pdd_po_srl, pdd_sch_dt, pdd_stag_qty) VALUES ($pdd_com, $pdd_unit, $pdd_fyr, $pdd_po_no, $pdd_po_srl, '$pdd_sch_dt[$j]', $pdd_stag_qty[$j])";
        ...
        }else if($passActn == 'a' && $frm_nm == 'worder') {  
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    <h1>
                        ...
                        <a href="exports/worder_export.php?ext=doc"><img src="img/word.png" width="35px"></a>  
                        <a href="exports/worder_export.php?ext=xls"><img src="img/xls.png" width="35px"></a>  
                        <a href="exports/worder_export.php?ext=pdf"><img src="img/pdf.png" width="35px"></a> -->                        
                    </h1>
                </section>

                <!-- Main content -->
                <section class="content">
                    <form role="form" name="worder" action="" method="post">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="box-body">
                                    <table cellpadding="1">
                                        <tr>
                                            <th>Password</th>
                                            <td>
                                                <input type="hidden" name="frm_nm" id="frm_nm" value="worder">
                                        ...
                                        <tr>            
                                            ...
                                            <td>
                                                <input type="text" name="poh_fyr" id="wo_poh_fyr" required maxlength="4" size="3" readonly>
                                            </td>    
                                        </tr>
                                        ...
                                        <tr>                                            
                                            ...
                                            <td>
                                                ...
                                                <input type="text" name="poh_po_dt" id="poh_po_dt" disabled readonly maxlength="10" size="8"  value="<?php echo date('d-m-Y'); ?>" required onblur="CheckWoPoNo()">
                                            </td> 
                                            <th>PO No.</th>
                                            <td><input type="text" name="poh_po_no" id="poh_po_no" readonly maxlength="5" size="3" required onblur="CheckWoPoNoDetails()">
                                            ...
                                            <td>
                                                <input type="text" name="poh_po_type" value="9" id="poh_po_type" readonly onblur="CheckPoTypeCd()" maxlength="3" size="3" onkeyup="CheckInputNumFormat(this.value)" required>
                                        ...
                                        <tr>      
                                            ...
                                            <th>GST Code
                                                <small>
                                                    <a href="includes/view_details.php?q=gstcd" onclick="window.open(this.href,'mywin','left=200,top=150,width=500,height=500,toolbar=1,resizable=0'); return false;" >(Help)</a>
                                                </small></th>
                                        ...
                                        <tr> 
                                            <th>Credit Days</th>
                                            <td><input type="text" name="poh_paycr_days" id="poh_paycr_days" readonly maxlength="4" size="4"></td>     
                                            <th>Disc.</th>
                                        ...
                                        <tr>
                                            <th>Srl No.</th>
                                            <td><input type="text" name="pod_po_srl" value="1" id="pod_po_srl" readonly maxlength="3" size="3" onfocus="checkWoPodSrlNo()" onblur="checkWoPodSrlNo()" required></td>
                                            <th>&nbsp;</th>
                                            <td>&nbsp;</td>
                                        </tr>
                                        <tr>
                                            <th>Item</th>
                                            <td colspan="8"><input type="text" name="pod_item" id="pod_item" readonly maxlength="7" size="7" onblur="CheckWoItemNo()">
                                            <span id="WoItemErrorCd"></span>
                                            </td>
                                        </tr>
                                        <tr> 
                                            <th>Rate</th>
                                        ...
                                        <tr>
                                            <th>Ord Qty.</th>
                                            <td colspan="8"><input type="text" name="pod_ord_qty" id="pod_ord_qty" readonly value="1.00"  maxlength="10" size="10" onblur="setNumberDecimal(this)" onkeyup="CheckWoOrdQty(this)">
                                            <span id="WoQtyErrorCd"></span>
                                            <span id="WoQtyValueErrorCd"></span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>Tolerance</th>
                                        ...
                                        <tr>
                                            <th>Chapter Id</th>
                                            <td><input type="text" name="pod_chpt_id" id="pod_chpt_id" readonly maxlength="10" size="4" style="text-transform: uppercase">
                                            </td>
                                        </tr>
                                        <tr><td colspan="8"><hr></td></tr>
                                        <tr>
                                            <td colspan="2">
                                                <table class="form-table" id="commercialFields">
                                                    ...
                                                    <tr valign="top" id="hidePdcOnupd">
                                                        ...
                                                        <td>
                                                            <input type="text" name="pdc_id[]" id="pdc_id" class="pdc_id" onblur="checkCommPdcVal(this)"  maxlength="2" size="1">
                                                        </td>
                                                        <td>
