...
if (isset($_POST['submit'])) {
	...
	$fr_gp_dept = $_POST['fr_gp_dept'];
	$to_gp_dept = $_POST['to_gp_dept'];
	...
	$sql = "SELECT 
			...
			cod.cod_prefix = 6
			WHERE gpa_com = $gpa_com AND gpa_unit = $gpa_unit AND gpa_ryn = '$gpa_ryn' AND gpa_dt  BETWEEN '$fr_gp_dt' AND '$to_gp_dt' AND gpa_dept  BETWEEN $fr_gp_dept AND $to_gp_dept";
...
?>
	<table border="0" width="100%">
		<tr class="bg-color">
			<th colspan="3">Department Wise Pending Gate Pass Status</th>
		</tr>
		<tr>
			<th  width="30%" style="text-align:left">Department Wise Pending Gate Pass Dt </th>
			<th> : </th>
			<td><?php echo date('d.m.Y', strtotime($fr_gp_dt)).' To '.date('d.m.Y', strtotime($to_gp_dt)); ?></td>
		</tr>
		<tr>
			<th  width="30%" style="text-align:left">Company Name </th>
			<th> : </th>
			<td><?php print_r($rows[0]['com_name']); ?></td>
		</tr>
		<tr>
			<th  width="30%" style="text-align:left">Company Unit </th>
			<th> : </th>
	...
	<?php   $i++; }  ?> 
		<tr>
			<td colspan="7">&nbsp;</td>
			<td colspan="3" style="text-align:right">Department Total Value</td>
