<?php 
    ...
    if (!empty($_POST['submit'])) {
        ...
        $pod_con = $_POST['pod_con'];
        // print_r($_POST);
        
        require_once('includes/cmp_pass_actn.php');

        if ($passActn == 'c' && $frm_nm == 'poshrtcl') { 

            for ($i=0; $i < count($pod_po_srl); $i++) { 
                if ($pod_con[$i] == 'Y' || $pod_con[$i] == 'y') {

                    $query2 = "UPDATE $UserComDbf.invac.podet SET pod_can_qty = $pod_can_qty[$i] WHERE pod_com = $pod_com AND pod_unit = $pod_unit AND pod_fyr = $pod_fyr[$i] AND pod_po_no = $pod_po_no[$i]  AND pod_po_srl = $pod_po_srl[$i] AND pod_item = '$pod_item[$i]'";
                                ...
                                <div class="box-body">
                                    <table cellpadding="1">
                                        <tr>
                                            <th>Password</th>
                                            <td>
                                                <input type="hidden" name="frm_nm" id="frm_nm" value="poshrtcl">
                                        ...
                                        <tr>
                                            ...
                                            <th>To Date</th>
                                            <td colspan="">
                                                <input type="text" name="to_poh_po_dt" id="to_poh_po_dt" readonly maxlength="10" size="10" required onchange="CheckPoShrtClrDetails()">
                                            </td>
                                            <td><span id="PoShrtClrErrorSpan"></span></td>
                                        </tr>
                                        <tr><td colspan="8"><hr></td></tr>
                                        <tr>
                                            <td colspan="6">
                                                <p id="pohPoShrtClrErrorSpan"></p>
                                            </td>
                                        </tr>
                                        <tr><td colspan="8"><hr></td></tr>
