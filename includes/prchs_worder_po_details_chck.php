<?php 
	...
	if(isset($_GET["q"])){
		...
		$WoPoDt = date('Y-m-d', strtotime($_GET['WoPoDt']));
		...
		$query1 = "SELECT * FROM $UserComDbf.invac.pohdr poh
					...
					WHERE poh_com = $ComCd AND	poh_unit = $UntCd AND poh_po_dt = '$WoPoDt' AND poh_fyr = $PoFinYr AND poh_po_no = $q AND poh_val_tag != 1";
		...
		if (!empty(odbc_result($result1, 1))) {
			...
			$pddData = "<table class='form-table'>";
				...
				foreach($rows as $row) {
						...
						<input type='text' name='pdd_sch_dt[]' id='pdd_sch_dt' maxlength='4' size='10' value='".date('d-m-Y', strtotime($row['pdd_sch_dt']))."'>
						<input type='text' name='pdd_stag_qty[]' id='pdd_stag_qty' maxlength='4' size='10' value='".$row['pdd_stag_qty']."'></td></tr>";
				}
			$pddData .= "<table>";
			...
			print(
				json_encode(
					array(
						...
						'poh_po_dt' => date('d-m-Y', strtotime(odbc_result($result1, 5))),
						...
						'poh_exp_dt' => date('d-m-Y', strtotime(odbc_result($result1, 30))),
						'poh_sys_dt' => date('d-m-Y', strtotime(odbc_result($result1, 31))),
						...
						'pod_comp_dt' => date('d-m-Y', strtotime(odbc_result($result1, 49))),
						'pod_sys_dt' => date('d-m-Y', strtotime(odbc_result($result1, 50))),
						'passActn' =>$passActn,
						'pdc_data' => $pdcData,
						'pdd_data' => $pddData
					)
